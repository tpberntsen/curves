...
% TODO find latex command for intersection and use below
\begin{equation}
    \sum_i \sum_{t \in T_j, t \in  [t_{i-1}, t_i) } p_i(t) S_{mult}(t)w(t)D(t) = 
...
\begin{multline}
    \sum_i \sum_{t \in T_i, t \in  [t_{i-1}, t_i)} ( \big[ z_{i-1} \sinh(\tau (t_i - t)) + z_i \sinh(\tau (t - t_{i-1})) \big] 
    / (\tau^2 \sinh(\tau h_i)) \\
+ (y_{i-1} - z_{i-1}/\tau^2)(t_i - t)/h_i + 
(y_i - z_i/\tau^2)(t - t_{i-1})/h_i + 
...
\begin{multline}
    \sum_i \biggl( z_i \sum_{t \in T_i, t \in  [t_{i-1}, t_i)} \sinh(\tau (t - t_{i-1}))
    / (\tau^2 \sinh(\tau h_i)) - 1/\tau^2(t - t_{i-1})/h_i \\
    + z_{i-1} \sum_{t \in T_i, t \in  [t_{i-1}, t_i)} \sinh(\tau (t_i - t))
    / (\tau^2 \sinh(\tau h_i)) - 1/\tau^2(t_i - t)/h_i \\
    + y_i \sum_{t \in T_i, t \in  [t_{i-1}, t_i)} (t - t_{i-1})/h_i \\
    + y_{i-1} \sum_{t \in T_i, t \in  [t_{i-1}, t_i)} (t_i - t)/h_i \biggr) \\
    = F_i \sum_{t \in T_i} w(t)D(t) - \sum_{t \in T_i} S_{add}(t) S_{mult}(t)w(t)D(t)
\end{multline}

...
\begin{eqnarray}
    \nonumber
    p_i(t_{i-1}) = \frac{z_{i-1} \sinh(\tau h_i) + z_i \sinh(0)}{\tau^2 \sinh(\tau h_i)}
    + \frac{(y_{i-1} - z_{i-1}/\tau^2) h_i }{h_i} \\
    \nonumber
    = \frac{z_{i-1}}{\tau^2} + y_{i-1} - \frac{z_{i-1}}{\tau^2} \\
    = y_{i-1}
\end{eqnarray}

\begin{eqnarray}
    \nonumber
    p_{i-1}(t_{i-1}) = \frac{z_{i-2} \sinh(0) + z_{i-1} \sinh(\tau h_{i-1})}{\tau^2 \sinh(\tau h_{i-1})}
    + \frac{(y_{i-1} - z_{i-1}/\tau^2) h_{i-1}}{h_{i-1}} \\
    \nonumber
    = \frac{z_{i-1}}{\tau^2} + y_{i-1} - \frac{z_{i-1}}{\tau^2} \\
    \nonumber
    = y_{i-1}
...
\begin{multline}
    p^\prime_i(t) = \frac{ z_{i-1} \cosh(\tau (t_i - t)) + z_i \cosh(\tau (t - t_{i-1}))}{\tau \sinh(\tau h_i)}  \\
        + \frac{y_i - y_{i-1} +  (z_{i-1} - z_i)/\tau^2}{h_i}
\end{multline}

For clarity, rearranging this to highlight the linearity with respect to the parameters:

\begin{multline}
    p^\prime_i(t) = z_{i-1} \biggl( \frac{ \cosh(\tau (t_i - t))}{\tau \sinh(\tau h_i)} + \frac{1}{h_i \tau^2} \biggr)
        + z_i \biggl( \frac{ \cosh(\tau (t - t_{i-1}))}{\tau \sinh(\tau h_i)} - \frac{1}{h_i \tau^2} \biggr) \\
